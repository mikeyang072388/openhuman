...
export const UnsubscribeApprovalCard: React.FC<Props> = ({ payload }) => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setStatus('pending');
      setErrorMsg(e?.message || t('unsubscribe.permissionError'));
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    <div className="border border-gray-200 dark:border-gray-800 rounded-lg p-4 my-2 bg-gray-50 dark:bg-gray-900">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {status === 'pending' && (
            <div className="flex gap-2 mt-4">
              <Button variant="primary" size="sm" onClick={handleApprove} disabled={isProcessing}>
                {isProcessing ? t('unsubscribe.processing') : t('unsubscribe.approve')}
              </Button>
          ...
          {status === 'denied' && (
            <div className="text-sm text-red-600 font-medium mt-3">{t('unsubscribe.denied')}</div>
          )}
        </div>
      </div>
    </div>
