...
export default function VoiceSetupModal({ onClose, skillStatus }: Props) {
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      setEnableError(error instanceof Error ? error.message : t('voiceSetup.failedToStart'));
    } finally {
      setIsEnabling(false);
    }
  };
      ...
      <div
        ...
        <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="voice-setup-title" className="text-sm font-semibold text-stone-900">{t('voiceSetup.title')}</h2>
              <p className="text-xs text-stone-500">
                {step === 'setup' && t('voiceSetup.setupSubtitle')}
                {step === 'enable' && t('voiceSetup.enableSubtitle')}
                {step === 'success' && t('voiceSetup.readyToGo')}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-5 py-4">
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 flex items-start gap-2">
                ...
                <div className="text-xs text-amber-700 leading-relaxed">
                  <p className="font-medium">{t('voiceSetup.sttModelNotReady')}</p>
                  <p className="mt-1">{t('voiceSetup.sttModelNotReadyDesc')}</p>
                </div>
              </div>

              <p className="text-xs text-stone-500 leading-relaxed">
                {t('voiceSetup.sttModelNotReadyHint')}
              </p>
          ...
          {step === 'enable' && (
            <div className="space-y-4">
              <div className="rounded-xl border border-sage-200 bg-sage-50 p-3 flex items-center gap-2">
                ...
                <span className="text-xs text-sage-700">{t('voiceSetup.sttModelReady')}</span>
              </div>

              <p className="text-xs text-stone-500 leading-relaxed">
                {t('voiceSetup.enableDescription')}
              </p>

              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5">
                  <span className="text-sm text-stone-700">{t('voiceSetup.hotkey')}</span>
                  <span className="text-xs font-mono text-stone-500">{serverStatus?.hotkey ?? 'Fn'}</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5">
                  <span className="text-sm text-stone-700">{t('voiceSetup.activation')}</span>
                  <span className="text-xs text-stone-500">{serverStatus?.activation_mode === 'push' ? t('voiceSetup.pushToTalk') : t('voiceSetup.tapToToggle')}</span>
                </div>
              </div>
              ...
              <button
                ...
                {isEnabling ? t('voiceSetup.starting') : t('voiceSetup.startVoiceServer')}
              </button>
            </div>
          )}
          ...
          {step === 'success' && (
            <div className="space-y-4 text-center py-2">
              ...
              <div>
                <h3 className="text-sm font-semibold text-stone-900">{t('voiceSetup.active')}</h3>
                <p className="text-center mt-1 text-xs text-stone-500 leading-relaxed">
                  {t('voiceSetup.activeDescription').replace('{hotkey}', serverStatus?.hotkey ?? 'Fn')}
                </p>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  {t('voiceSetup.customizeSettings')}
                </button>
                <button
                  type="button"
                  onClick={onClose}
