  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold text-stone-900">
        {t('webhooks.composeio.countLabel').replace('{count}', String(entries.length))}
      </h3>
      <div className="space-y-3">
        {entries.map(entry => (
          <article
            ...
            <dl className="mt-3 grid gap-2 text-sm text-stone-700 md:grid-cols-2">
              <div>
                <dt className="text-xs uppercase tracking-wide text-stone-400">{t('webhooks.composeio.metadataId')}</dt>
                <dd className="font-mono text-xs break-all">{entry.metadata_id}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wide text-stone-400">{t('webhooks.composeio.metadataUuid')}</dt>
                <dd className="font-mono text-xs break-all">{entry.metadata_uuid}</dd>
              </div>
            </dl>

            <div className="mt-3">
              <div className="mb-2 text-xs uppercase tracking-wide text-stone-400">{t('webhooks.composeio.payload')}</div>
