...
const ServiceBlockingGate = ({ children }: ServiceBlockingGateProps) => {
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      setRestartError(t('serviceBlockingGate.retryFailed'));
    } catch {
      setRestartError(t('serviceBlockingGate.retryFailed'));
    } finally {
    ...
    <>
      ...
      <div className="fixed inset-0 z-[10000] bg-stone-950/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="w-full max-w-xl rounded-2xl border border-coral-500/30 bg-stone-900 p-6 shadow-2xl">
          <h2 className="text-xl font-semibold text-white">{t('serviceBlockingGate.unavailableTitle')}</h2>
          <p className="mt-2 text-sm text-stone-300">
            {t('serviceBlockingGate.unavailableDescription')}
          </p>
          <p className="mt-2 text-sm text-stone-400">
            {t('serviceBlockingGate.downloadToRestore')}
          </p>
          ...
          <div className="mt-5 flex gap-3">
            <button
              ...
              {isRestarting ? t('serviceBlockingGate.retrying') : t('serviceBlockingGate.retryCore')}
            </button>
            <button
              type="button"
              onClick={handleDownloadLatest}
              className="rounded-lg bg-coral-500 px-4 py-2 text-sm font-medium text-white hover:bg-coral-600">
              {t('serviceBlockingGate.downloadLatestVersion')}
            </button>
          </div>
        </div>
      </div>
