...
export default function RewardsCommunityTab({
  ...
  const discordMembershipLabel = (snapshot: RewardsSnapshot | null): string => {
    if (!snapshot) return t('rewards.community.membership.waiting');
    switch (snapshot.discord.membershipStatus) {
      case 'member':
        return t('rewards.community.membership.joined');
      case 'not_in_guild':
        return t('rewards.community.membership.linkedNotInServer');
      case 'not_linked':
        return t('rewards.community.membership.notLinked');
      default:
        return t('rewards.community.membership.unavailable');
    }
  };
    ...
    <>
        ...
        <div className="relative z-10 space-y-4">
          <div className="space-y-2">
            <h1 className="text-2xl font-bold tracking-tight text-white">
              {t('rewards.community.title')}
            </h1>
            <p className="text-sm font-medium leading-relaxed text-white/90">
              {t('rewards.community.description')}
            </p>
          </div>
      ...
      {error ? (
        <div
          ...
          <span>
            {t('rewards.community.error').replace('{error}', error ?? '')}
          </span>
          {onRetry ? (
            <button
              type="button"
        ...
        <section className="rounded-[1.25rem] bg-[#f2f4f6] p-4 text-sm text-stone-600">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <span className="font-semibold text-stone-900">
              {snapshot
                ? `${snapshot.metrics.currentStreakDays} ${t('rewards.community.days')}`
