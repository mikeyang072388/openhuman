      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-amber-700">
            {t('homeBanners.promoCreditsMessage').replace('{amount}', formatUsd(promoCredits))}
          </p>
          <p className="mt-1 text-sm leading-relaxed text-amber-600">
            {t('homeBanners.giveASpin')}{' '}
            <button
              type="button"
              onClick={() => {
                void openUrl(BILLING_DASHBOARD_URL);
              ...
              {t('homeBanners.getSubscription')}
            </button>{' '}
            {t('homeBanners.tenXUsage')}
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="relative mb-3 mt-3 rounded-2xl border border-orange-200 bg-gradient-to-r from-orange-50 via-amber-50 to-orange-50 px-4 py-4 text-left shadow-soft">
      {onDismiss && (
        <button
          type="button"
          onClick={onDismiss}
          aria-label={t('homeBanners.dismissEarlyBird')}
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm leading-relaxed text-orange-600">
            ...
            <button
              ...
              className="cursor-pointer border-b border-amber-700 border-dashed font-bold text-amber-700 hover:text-amber-800">
              {t('homeBanners.firstSubscription')}
            </button>{' '}
          </p>
        </div>
      </div>
  ...
  return (
    <button
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-semibold">{t('homeBanners.joinDiscord')}</div>
          <div className="mt-0.5 text-sm text-[#5E66BC]">
            {t('homeBanners.discordDescription')}
          </div>
        </div>
      </div>
    </button>
